...
* **多维度语义匹配 (Multi-Layer Matching)：** 底层支持 4 个维度的向量空间检索（英文、中文扩展词、维基释义、中文核心词），大幅提升模糊描述的命中率。
* **智能分词控制 (Segmentation)：** 支持一键开启/关闭智能分词。开启时自动提取长句中的概念进行组合检索；关闭后严格执行整句语义匹配。
* **上下文关联推荐 (Co-occurrence Recommendation)：** 引擎内置标签共现数据。当你选中某些标签后，系统会自动推荐在 Danbooru 图库中经常与它们一同出现的相关标签，辅助补全画面细节。
...
* **热度权重（0~1）：** 控制标签在 Danbooru 站内发帖量（Post Count）对最终排序的影响。数值越高，越倾向于推荐常用、高频的标签（推荐默认值为 0.15）。
...
![长句搜索示例](https://akizukipic.oss-cn-beijing.aliyuncs.com/img/202602082126523.png)

### 2. 关键词精确查找
适用于对某个概念有印象，但不知晓其准确 Danbooru 英文标签的场景。

**示例输入：**
> 假肢

**建议参数：** Top K: 5 | 结果上限: 10 | 热度权重: 0.15

![精准查找示例](https://akizukipic.oss-cn-beijing.aliyuncs.com/img/202602082129286.png)
...
![模糊查找示例](https://akizukipic.oss-cn-beijing.aliyuncs.com/img/202602082132644.png)
