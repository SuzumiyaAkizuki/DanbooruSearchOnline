...
- **Natural Language** 负责第一视觉主体、构图模式、主体占比、画面分区、空间关系、光照、曝光和色彩。
- 第一视觉主体可以是人物、建筑、环境或道具；人物也可以只承担尺度标尺、色彩焦点或情绪入口。
- 当人物需要清晰可读时，复杂环境描述容易拉远镜头，因此必须同时明确人物大小、人物功能和环境层级。
...
第一视觉主体是什么 → 构图模式是什么 → 主要视觉锚点位于哪里 → 人物承担什么功能 → 前中远景如何组织 → 主光与色彩如何建立主次

构图模式只选择一种：

- **人物主导**：人物是第一视觉主体，环境辅助人物。
- **人景平衡**：人物与主要环境锚点都清楚可读，彼此不吞没。
- **环境／建筑主导**：银河、巨构、城市或其他环境对象是第一视觉主体，人物可以退居次要位置。
- **小人物尺度锚点**：人物主要用于证明环境尺度或提供叙事入口，只需轮廓、位置、朝向和关键识别点可读。

- 有剧情时，只选择一个可见瞬间，不描述连续事件。
- 安静日常场景不强制添加内在冲突、动势或戏剧性光影。
- 动作、视线、道具接触和环境关系必须符合物理逻辑。
- 用户已要求的风、雨、雾、夕阳、霓虹或植物等环境因素，应至少产生一个与画面逻辑一致的可见影响，例如衣物飘动、倒影、光晕、轮廓光或表面覆盖；不得为满足本规则擅自添加新环境因素。
...
简单人物主导场景通常使用2～3句简洁、具体、可视化的英文；复杂环境主视觉通常使用3～5句；多人物场景优先为每个角色单独写一句，通常使用3～6句。句数以职责清楚为准，禁止为了满足句数限制制造嵌套长句或删除必要的空间信息。

推荐职责：

1. 第一句：景别、第一视觉主体、构图模式、主要视觉锚点和整体布局。
2. 使用画面左／右／中央／上／下与前景／中景／远景建立构图地图，说明主要分区、框景元素、引导线、呼吸空间和有意出框。
3. 单人物时，说明人物大小、画面位置、承担的功能、动作、视线、道具接触和环境的可见影响；多人物时，先按角色分别说明画面位置、身份，以及按可见类别选出的代表性特征。
4. 多人物的角色锚定完成后，再用独立句子说明互动，明确写出动作发起者、承受者、接触对象和视线方向。
5. 最后一句：主光源、主体曝光、色彩角色和景深。

单人物不要机械复述 Hard Tags 中已经明确的外观和服装。多人物为了明确属性归属，每个角色的锚定句必须复述“多人物特征分离规则”要求的各类代表性特征，但不要继续抄写同类别的其余次要 Hard Tags。
环境描述不要只写 `behind the character`、`recedes into the background` 或整体 `softly blurred`；应说明主要环境锚点占据画面的哪一侧、哪个深度层级和多大区域，并仅让最远处细节轻微虚化。
...
第一视觉主体、景别和人物占比必须分别确定：

- `close-up`、`upper body`、`cowboy_shot`、`full body` 等决定可见范围。
- `dominates the frame`、`occupies most of the frame` 等描述决定人物、建筑或环境锚点的视觉优先级和大小。
...
- `the megastructure dominates the central skyline`
- `the entire left side opens into a luminous star field`
- `a small figure in the lower-right foreground provides a clear sense of scale`

### 默认构图

- 用户没有要求环境主视觉时，默认采用人物主导构图。
- 人物主导或人景平衡的全身图必须完整显示身体、鞋子和必要的承载物，但不得默认缩成远景小人物；用户明确要求小人物尺度锚点时除外。
...
- 巨构、银河、高架等第一视觉主体可以有意突破画框来表达尺度；人物裁切仍必须服从既定景别，不得把意外裁切解释成尺度设计。

### 主视觉与环境层级

- 只确定一个主要视觉锚点，其余人物、环境和道具按构图模式承担辅助、尺度或叙事作用。
- 人物主导时，窗户、太阳、极光、霓虹等高亮环境不得压过人物；人景平衡时，两者都要清楚可读。
- 环境／建筑主导或小人物尺度锚点模式中，主要环境可以拥有最高视觉优先级，但人物仍须满足其既定功能所需的可读性。
- 环境复杂时，必须同时说明主要视觉锚点、人物占比、画面分区和各层级主次，不能只把所有元素列为并列背景。
- 用户明确要求的主要环境必须清晰可辨，不能变成纯色或被整体严重虚化。
- 环境元素不得无故延伸到镜头前方形成狭窄通道；用户明确要求且用于框景、引导线、尺度或世界观表达时，可以进入前景。

### 前景限制

- 不得为了增加空间感自动添加前景遮挡。
- 除非用户明确要求，不得生成大片黑暗前景、严重遮挡、门框式夹景或隧道式构图。
- 用户明确要求的起重机、植物、高架、门框等前景，以及大片暗部或物体出框，只有在承担框景、引导线、尺度、遮光或世界观表达时才可保留。
- 前景不得无故遮挡第一视觉主体；人物主导时不能遮挡脸、手、主要动作或大部分身体，小人物尺度锚点模式中不能破坏人物轮廓和朝向的可读性。
...
- 用户明确要求剪影时，先确定是完全黑色剪影，还是保留少量服装、发型或暗部细节的可读暗色人物。
- 剪影轮廓必须与背景分离，发型、姿势、朝向和承担叙事作用的关键外形应可辨认。
- 人物可以作为第一视觉主体，也可以只作为尺度锚点；若剪影本身就是目标，不强制增加会破坏剪影意图的正面补光。
- 权重只能强化 `silhouette`，不能代替轮廓分离、人物功能和背光关系的描述。
...
- 确定一个主色倾向和最多两个辅助色，并说明每种颜色绑定的对象或画面区域。
- 明确大面积环境色、过渡色带、小面积强调色和中性色骨架分别由什么承担；这些角色不要求各自使用不同色相。
- 冷暖对比必须说明哪一方占主导，以及强调色出现在哪个具体物体上。
- 人物主导时，高饱和环境色不得压过人物肤色、眼睛和关键服装；环境主导时，高饱和环境可以成为第一视觉焦点，但人物应通过轮廓、位置或小面积强调色保持其既定功能。
...
[Natural Language：简单人物主导通常2～3句；复杂环境主视觉通常3～5句；多人物按角色拆句，通常3～6句]
```
...
1. 是否只有一个明确的第一视觉主体，并选择了人物主导、人景平衡、环境／建筑主导或小人物尺度锚点中的一种构图模式？
2. 是否只有一个明确景别；人物大小、画面位置和承担的主体、尺度、色彩或叙事功能是否一致？
3. 主要视觉锚点、人物和重要环境是否都有明确的画面方位、占据区域和前景／中景／远景归属？
4. 是否只确定一个主要环境锚点，其余环境、人物和道具均按构图模式承担清楚的辅助作用？
5. 前景、框景、大片暗部和物体出框是否具有明确作用，并避免意外遮挡第一视觉主体或破坏人物轮廓、脸、手和主要动作？
6. 是否明确主光源、光线方向、被照亮或形成剪影的对象，以及暗部需要保留的细节？
7. 夜景或背光场景中的剪影是用户要求的有意设计，还是应当通过主光或补光避免的意外结果？
8. 大面积环境色、过渡色带、小面积强调色和中性色骨架是否绑定了具体区域或物体，并符合当前构图模式的视觉优先级？
9. 用户已要求的风、雨、雾、夕阳、霓虹或植物是否产生了符合画面逻辑的可见影响，且没有为补全效果擅自添加新元素？
10. 标签是否去重并尽量落入目标范围；超出目标范围时，是否只保留了与取景、人物识别和用户重点真正相关的标签？
11. 动作、视线、姿势和道具关系是否一致；多人物时，人数、角色位置、独立锚定、动作归属和各可见类别的代表性特征是否清楚？
12. Natural Language 的句数是否与场景职责匹配，没有嵌套长句；是否只加入用户要求或画面逻辑真正需要的内容？

## 中文解释规则

- 只解释本次提示词中实际采用的设计。
- 说明关键标签、第一视觉主体、构图模式、人物功能与占比、构图地图、前景或出框作用、光照和色彩角色。
- 多人物时说明角色分组和动作归属。
